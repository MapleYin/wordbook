...
export function TopNav({ signedIn }: { signedIn: boolean }) {
  ...
  return (
    <header className="mx-auto flex w-full max-w-2xl items-center justify-between px-6 py-6 font-sans text-sm">
      <Link href="/" className="font-serif text-lg text-ink">
        English Words Collector
      </Link>
      {signedIn && (
        <nav className="flex items-center gap-4 text-ink/70">
          <Link href="/review" className="hover:text-ink">Review</Link>
